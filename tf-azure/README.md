...
`myorg/terraform-modules`. This repo wires together those platform modules via a
...
platform Terraform modules. All are sourced from `myorg/terraform-modules` via
SSH deploy key:
...
`myorg/reusable-workflows`. Product teams do not write pipeline logic — they

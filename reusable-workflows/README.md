# myorg/reusable-workflows — Terraform
...
| `TF_MODULES_DEPLOY_KEY` | Repository | Read-only SSH key for `myorg/terraform-modules` |
...
`myorg/reusable-workflows`, they cannot enforce your repo's approval rules.
...
deploy-prod:
  needs: gate-prod
  uses: myorg/reusable-workflows/.github/workflows/tf-deploy.yml@main
  ...
```
...
jobs:
  # ── Always ────────────────────────────────────────────────────────────────────
  validate:
    uses: myorg/reusable-workflows/.github/workflows/tf-validate.yml@main
    secrets:
  ...
  changes:
    ...
    uses: myorg/reusable-workflows/.github/workflows/tf-changes.yml@main
  ...
  plan-dev:
    ...
    uses: myorg/reusable-workflows/.github/workflows/tf-plan.yml@main
    with:
      environment:    dev
...
```yaml
uses: myorg/reusable-workflows/.github/workflows/tf-plan.yml@v1.2.0
```
